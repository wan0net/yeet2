...
export async function POST(_: Request, { params }: { params: Promise<{ id: string; blockerId: string }> }) {
  ...
  try {
    const response = await fetch(upstreamUrl(`/projects/${projectId}/blockers/${blockerId}/github-issue`), {
      method: "POST",
      cache: "no-store",
      headers: {
